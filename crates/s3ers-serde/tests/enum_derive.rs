...
#[s3ers_enum(rename_all = "snake_case")]
enum MyEnum {
    First,
    Second,
    #[s3ers_enum(rename = "m.third")]
    Third,
    HelloWorld,
...
fn as_ref_str() {
    ...
    assert_eq!(
        MyEnum::_Custom(PrivOwnedStr("HelloWorld".into())).as_ref(),
        "HelloWorld"
    );
}
...
fn display() {
    ...
    assert_eq!(
        MyEnum::_Custom(PrivOwnedStr("HelloWorld".into())).to_string(),
        "HelloWorld"
    );
}
...
fn from_string() {
    ...
    assert_eq!(
        MyEnum::from("HelloWorld"),
        MyEnum::_Custom(PrivOwnedStr("HelloWorld".into()))
    );
}
...
fn serialize() {
    ...
    assert_eq!(
        to_json_value(MyEnum::HelloWorld).unwrap(),
        json!("hello_world")
    );
    assert_eq!(
...
fn deserialize() {
    assert_eq!(
        from_json_value::<MyEnum>(json!("first")).unwrap(),
        MyEnum::First
    );
    assert_eq!(
        from_json_value::<MyEnum>(json!("hello_world")).unwrap(),
        MyEnum::HelloWorld
    );
    assert_eq!(
