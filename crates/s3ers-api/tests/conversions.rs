...
use s3ers_api::{
    s3ers_api, IncomingRequest as _, OutgoingRequest as _, OutgoingRequestAppserviceExt,
    SendAccessToken,
};
...
fn request_serde() {
    let req = Request {
        ...
        q2: 55,
        bar: "barVal".to_owned(),
        user: user_id!("@bazme:s3ers.io").to_owned(),
    };
...
fn request_with_user_id_serde() {
    ...
    let user_id = user_id!("@_virtual_:s3ers.io");
    let http_req = req
    ...
    assert_eq!(
        query,
        "q1=query_param_special_chars+%25%2F%26%40%21&q2=55&user_id=%40_virtual_%3As3ers.io"
    );
}

mod without_query {
    use super::{s3ers_api, user_id, OutgoingRequestAppserviceExt, SendAccessToken, UserId};
    ...
    fn request_without_query_with_user_id_serde() {
        ...
        assert_eq!(query, "user_id=%40_virtual_%3As3ers.io");
    }
}
