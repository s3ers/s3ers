...
impl Request {
    pub fn expand_incoming(&self, s3ers_api: &TokenStream) -> TokenStream {
        ...
        let (parse_query, query_vars) = if let Some(field) = self.query_map_field() {
            ...
            let parse = quote! {
                let request_query: <RequestQuery as #s3ers_serde::Outgoing>::Incoming =
                    #s3ers_serde::urlencoded::from_str(
                        &request.uri().query().unwrap_or("")
                    )?;
        ...
        quote! {
            #[automatically_derived]
            #[cfg(feature = "server")]
            impl #s3ers_api::IncomingRequest for #incoming_request_type {
