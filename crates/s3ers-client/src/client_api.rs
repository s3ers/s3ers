...
use s3ers_client_api::r0::{
    account::register::{self, RegistrationKind},
    session::login::{self, LoginInfo},
    sync::sync_events,
    uiaa::UserIdentifier,
};
use s3ers_common::presence::PresenceState;
use s3ers_identifiers::DeviceId;
...
impl<C: HttpClient> Client<C> {
    ...
    ) -> Result<login::Response, Error<C::Error, s3ers_client_api::Error>> {
        let response = self
    ...
    pub async fn register_guest(
        &self,
    ) -> Result<register::Response, Error<C::Error, s3ers_client_api::r0::uiaa::UiaaResponse>> {
        let response = self
    ...
    ) -> impl Stream<Item = Result<sync_events::Response, Error<C::Error, s3ers_client_api::Error>>> + 'a
    {
        try_stream! {
            loop {
                let response = self
