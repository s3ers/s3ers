...
pub fn expand_derive_outgoing(input: DeriveInput) -> syn::Result<TokenStream> {
    ...
    derives.push(if derive_deserialize {
        quote! { #s3ers_serde::exports::serde::Deserialize }
    } else {
    ...
    match data {
        ...
        DataKind::Enum(mut vars) => {
            ...
            Ok(quote! {
                ...
                impl #original_impl_gen #s3ers_serde::Outgoing for #original_ident #original_ty_gen {
...
fn impl_outgoing_with_incoming_self(input: &DeriveInput, s3ers_serde: &TokenStream) -> TokenStream {
    ...
    quote! {
        #[automatically_derived]
        impl #impl_gen #s3ers_serde::Outgoing for #ident #ty_gen {
            type Incoming = Self;
        }
    }
}
